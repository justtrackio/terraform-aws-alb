...
| <a name="module_alb_label"></a> [alb\_label](#module\_alb\_label) | justtrackio/label/null | 0.26.0 |
| <a name="module_glue_label"></a> [glue\_label](#module\_glue\_label) | justtrackio/label/null | 0.26.0 |
...
|------|-------------|------|---------|:--------:|
| <a name="input_access_logs"></a> [access\_logs](#input\_access\_logs) | Map containing access logging configuration for load balancer. | <pre>object({<br>    bucket  = string<br>    prefix  = string<br>    enabled = bool<br>  })</pre> | `null` | no |
...
| <a name="input_aws_account_id_monitoring"></a> [aws\_account\_id\_monitoring](#input\_aws\_account\_id\_monitoring) | AWS account id for monitoring account where the s3 bucket will be | `string` | `null` | no |
...
| <a name="input_glue_database_name"></a> [glue\_database\_name](#input\_glue\_database\_name) | Set which database name for glue tables should be used, uses {module.this.organizational\_unit}\_alb\_logs by default | `string` | `null` | no |
...
| <a name="input_http_tcp_listeners"></a> [http\_tcp\_listeners](#input\_http\_tcp\_listeners) | A list of maps describing the HTTP listeners or TCP ports for this ALB. Required key/values: port, protocol. Optional key/values: target\_group\_index (defaults to http\_tcp\_listeners[count.index]) | `any` | <pre>[<br>  {<br>    "action_type": "fixed-response",<br>    "fixed_response": {<br>      "content_type": "text/plain",<br>      "message_body": "Fixed message",<br>      "status_code": "404"<br>    },<br>    "port": 80,<br>    "protocol": "HTTP"<br>  }<br>]</pre> | no |
...
| <a name="input_ip_address_type"></a> [ip\_address\_type](#input\_ip\_address\_type) | The type of IP addresses used by the subnets for your load balancer. The possible values are ipv4 and dualstack. | `string` | `"dualstack"` | no |
| <a name="input_label_key_case"></a> [label\_key\_case](#input\_label\_key\_case) | Controls the letter case of the `tags` keys (label names) for tags generated by this module.<br>Does not affect keys of tags passed in via the `tags` input.<br>Possible values: `lower`, `title`, `upper`.<br>Default value: `title`. | `string` | `null` | no |
| <a name="input_label_order"></a> [label\_order](#input\_label\_order) | The order in which the labels (ID elements) appear in the `id`.<br>Defaults to ["namespace", "environment", "stage", "name", "attributes"].<br>You can omit any of the 6 labels ("tenant" is the 6th), but at least one must be present. | `list(string)` | `null` | no |
| <a name="input_label_orders"></a> [label\_orders](#input\_label\_orders) | Overrides the `labels_order` for the different labels to modify ID elements appear in the `id` | <pre>object({<br>    ec2  = optional(list(string)),<br>    glue = optional(list(string), ["namespace", "environment", "name"]),<br>  })</pre> | `{}` | no |
...
| <a name="input_security_group_rules"></a> [security\_group\_rules](#input\_security\_group\_rules) | Security group rules to add to the security group created | `any` | <pre>{<br>  "egress_all": {<br>    "cidr_blocks": [<br>      "0.0.0.0/0"<br>    ],<br>    "from_port": 0,<br>    "ipv6_cidr_blocks": [<br>      "::/0"<br>    ],<br>    "protocol": "-1",<br>    "to_port": 0,<br>    "type": "egress"<br>  },<br>  "ingress_all_http": {<br>    "cidr_blocks": [<br>      "0.0.0.0/0"<br>    ],<br>    "description": "HTTP web traffic",<br>    "from_port": 80,<br>    "ipv6_cidr_blocks": [<br>      "::/0"<br>    ],<br>    "protocol": "tcp",<br>    "to_port": 80,<br>    "type": "ingress"<br>  },<br>  "ingress_all_https": {<br>    "cidr_blocks": [<br>      "0.0.0.0/0"<br>    ],<br>    "description": "HTTPS web traffic",<br>    "from_port": 443,<br>    "ipv6_cidr_blocks": [<br>      "::/0"<br>    ],<br>    "protocol": "tcp",<br>    "to_port": 443,<br>    "type": "ingress"<br>  }<br>}</pre> | no |
